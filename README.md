...
| <a name="input_security_group_enable_ssh_access"></a> [security\_group\_enable\_ssh\_access](#input\_security\_group\_enable\_ssh\_access) | (Optional) Determines if SSH access is allowed from configured CIDR ranges.<br/><br/>When set to false, the SSH (port 22) firewall rule is omitted from the ingress CIDR rules.<br/><br/>ex:<pre>security_group_enable_ssh_access = true</pre> | `bool` | `true` | no |
